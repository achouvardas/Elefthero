...
def invoice_pdf(invoice_id):
    ...
    if viva_payment:
        try:
            ...
            viva_lines = ["Πληρωμή με Viva.com", f"Transaction ID: {viva_event.get('TransactionId', viva_payment.transaction_id)}", f"TID: {viva_event.get('TerminalId', viva_payment.terminal_id)} · RRN: {viva_event.get('RetrievalReferenceNumber', '-')} · Order Code: {viva_event.get('OrderCode', '-')}", f"Card Number: {viva_event.get('CardNumber', '-')} · Authorization ID: {viva_event.get('AuthorizationId', '-')}"]
            pdf_notes = (pdf_notes + "\n" if pdf_notes else "") + "\n".join(viva_lines)
